...
if (isset($_GET['id']) && is_numeric($_GET['id']) && ($_GET['id'] > 0) ) { // Display the quote in a form:
	...
	if ($r = mysqli_query($mysqli, $query)) { // Run the query
		...
		print '<p class="error">Could not retrieve the quote because:<br />' . mysqli_error($mysqli) . 
		'.</p><p>The query being run was: ' . $query . '</p>';
	}

} elseif (isset($_POST['id']) && is_numeric($_POST['id']) && ($_POST['id'] > 0) ) { // Handle the form after submit

	// Define the query:
	$query = "DELETE FROM quotes WHERE quote_id={$_POST['id']} LIMIT 1";
	$r = mysqli_query($mysqli, $query); // Execute the query.

	// Report on the result:
	if (mysqli_affected_rows($mysqli) == 1) {
		print '<p>The quote entry has been deleted.</p>';
	} else { // Couldn't get the information
		print '<p class="error">Could not delete the blog entry because:<br />' . mysqli_error($mysqli) . 
		'.</p><p>The query being run was: ' . $query . '</p>';
	}
	...
	print '<p class="error">Could not delete the blog entry because:<br />' . mysqli_error($mysqli) . '.</p><p>The query being run was: '
	. $query . '</p>';
}
